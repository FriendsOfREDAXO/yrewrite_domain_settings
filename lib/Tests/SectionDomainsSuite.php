 ...
 * installation that never touches the setting working as before. The whole
 * point of the setting is that it only narrows what the editing page offers:
 * the values stay where they are and every read keeps answering, so taking a
 * domain away from a tab can hide an input but never a value. That promise is
 * what testTheAssignmentDoesNotReachTheReadPath() is here for.
 *
...
final class SectionDomainsSuite extends AbstractSuite
{
    ...
    private const DOMAIN_MERGE = Fixtures::DOMAIN_ID + 2;
    ...
    /**
     * The central promise: the assignment is a view filter.
     *
     * Rows stay where they are, and every read keeps answering - taking a
     * domain away from a tab hides an input in the backend and nothing else.
     */
    public function testTheAssignmentDoesNotReachTheReadPath(): void
    {
        $domains = array_keys(Backend::getAllDomains());
        ...
        $this->fixtures->setValues($clangId, ['selftest_text' => 'trotz zuordnung lesbar']);
        $this->assign($table, [$domains[0]]);

        Assert::false(
            Backend::isSectionVisibleForDomain($table, Fixtures::DOMAIN_ID),
            'precondition: the tab is hidden for the domain the value belongs to',
        );

        Assert::same(
            'trotz zuordnung lesbar',
     ...
     *
     * The two tabs are assigned to different domains on purpose: they share a
     * field name, and only that assignment keeps build() from reporting it as
     * a duplicate.
     */
    public function testAFilledValueBeatsAnEmptyOneFromAnotherTab(): void
    {
        ...
        if (count($domains) < 2) {
            Assert::skip('needs two domains to keep the shared field name out of the duplicate report');
        }
        ...
        try {
            ...
            $this->write($a, self::DOMAIN_MERGE, $clangId, [$field => '']);
            $this->write($b, self::DOMAIN_MERGE, $clangId, [$field => 'aus b']);

            Assert::same(
                'aus b',
                DomainSettings::get($field, null, self::DOMAIN_MERGE, $clangId),
            ...
            $this->write($a, self::DOMAIN_MERGE, $clangId, [$field => 'aus a']);
            $this->write($b, self::DOMAIN_MERGE, $clangId, [$field => '']);
    ...
    public function testSectionHasValuesLooksAtOneDomainOnly(): void
    {
        ...
        Assert::false(
            Backend::sectionHasValues($table, self::DOMAIN_MERGE),
    ...
    private function createSection(string $label, string $field): string
    {
